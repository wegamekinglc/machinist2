...
namespace {
    ...
    bool IsIngredientStart(const string& line) // see if it might be an ingredient
    {
        ...
        return !line.empty() && FIRST.find(line[0]) != string::npos;
    }

    bool IsCO(const string& line) { return line.find("c/o") == 0 || line.find("&co") == 0; }

    bool IsNewPart(const string& line) { return line.substr(0, 6) == "+part " || line.substr(0, 6) == "&part "; }
    bool IsStopPart(const string& line) { return line.substr(0, 5) == "-part"; }
    string PartName(const string& line) { return line.substr(6); }

    bool StartsWith(const string& line, const string& token) { return line.find(token) == 0; }
    ...
    vector<string>::const_iterator AddIngredientsAndActions(Info_* info,
                                                            Info_* root,
