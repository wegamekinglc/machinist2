...
namespace {
    ...
    bool
    IsIngredientStart(
            const std::string &line)                                              // see if it might be an ingredient
    {
    ...
    bool
    IsCO(const std::string &line) {
        return line.find("c/o") == 0 || line.find("&co") == 0;
    }


    bool
    IsNewPart(const std::string &line) {
        return line.substr(0, 6) == "+part " || line.substr(0, 6) == "&part ";
    }


    bool
    IsStopPart(const std::string &line) {
        return line.substr(0, 5) == "-part";
    }


    std::string
    PartName(const std::string &line) {
    ...
    std::string
    StandardMeasure(const std::string &src) {
        std::string retval(src);
        auto lower = [](char c) { return std::tolower(c); };                                 // discard overloading
        std::transform(retval.begin(), retval.end(), retval.begin(), lower);
    ...
    Handle_ <Info_>
    ParseIngredient(const std::string &line,
                    const Info_ *parent,
                    const Info_ *root) {
        assert(!line.empty());
        ...
        if (!number.empty() && !rest.empty() &&
            rest[0] == '~')                             // glue modifier to the number, e.g. 3~heaping
        {
        ...
        std::unique_ptr<Info_> retval(new Info_(parent, root, TrimWhitespace(rest)));
        if (!measure.empty()) {
            retval->children_.insert(std::make_pair(MEASURE, Info::MakeLeaf(retval.get(), root, measure)));
        }
        if (!number.empty()) {
            retval->children_.insert(std::make_pair(NUMBER, Info::MakeLeaf(retval.get(), root, number)));
        }
        if (!processed.empty()) {
            retval->children_.insert(std::make_pair(PROCESSED, Info::MakeLeaf(retval.get(), root, processed)));
        }
    ...
    AddIngredientsAndActions(Info_ *info,
                             Info_ *root,
                             std::vector<std::string>::const_iterator line,
                             std::vector<std::string>::const_iterator end) {
        // starts with ingredients
        while (line != end && IsIngredientStart(*line)) {
            info->children_.insert(std::make_pair(INGREDIENT, ParseIngredient(*line++, info, root)));
        }
        ...
        while (++line != end && !IsNewPart(*line) && !IsStopPart(*line)) {
            info->children_.insert(std::make_pair(ACTION, Info::MakeLeaf(info, root, *line)));
        }
    ...
    struct ParseRecipe_ : Info::Parser_ {
        ...
        operator()(const std::string &info_name, const std::vector<std::string> &content) const {
            ...
            if (line != content.end() && IsCO(*line)) {
                retval->children_.insert(std::make_pair(CO, ParseCO(*line++, retval.get(), retval.get())));
            }

            // have to check for parts
            while (line != content.end() && IsNewPart(*line)) {
                std::unique_ptr<Info_> part(new Info_(retval.get(), retval.get(), PartName(*line)));
                line = AddIngredientsAndActions(part.get(), retval.get(), ++line, content.end());
                retval->children_.insert(std::make_pair(PART, part.release()));
            }
            std::cout << "Finished children at line " << (line - content.begin()) << "\n";
