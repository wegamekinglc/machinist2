...
namespace {
    std::map<std::string, const Info::Parser_ *> &
    TheParsers() {
        static std::map<std::string, const Info::Parser_ *> RETVAL;
        return RETVAL;
    }
...
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ //
    ...
    Info_ *
    XParseRaw(const Info_ *parent,
              const Info_ *root,
              const std::string &info_name,
              std::vector<std::string>::const_iterator &line,
              std::vector<std::string>::const_iterator end,
              int min_offset) {
        std::unique_ptr<Info_> retval(new Info_(parent, root, info_name));
        for (;;) {
            int indent = line == end ? -1 : Indent(*line);
            if (indent < min_offset)                                            // pop out to parent
            {
            ...
            retval->children_.insert(make_pair(childName, Handle_<Info_>(
                    XParseRaw(retval.get(), root, childVal, ++line, end, indent + 1))));
        }
    }


    Info_ *
    ParseRaw(const std::string &info_name, const std::vector<std::string> &content) {
        ...
        return XParseRaw(0, 0, info_name, start, content.end(), 0);
    }


    void
    CheckBackPointers(const Info_ &info,
...
void
Info::
RegisterParser(const std::string &type, const Info::Parser_ &parser) {
...
operator()(const Info_ &here, bool quiet) const {
    ...
    for (auto pc = childNames_.begin(); pc != childNames_.end(); ++pc) {
        ...
        if (*pc == "..") {
            REQUIRE(retval->parent_, "No parent to navigate to in " + ShowPath(here));
            retval = retval->parent_;
        } else if (*pc != ".") {
            auto cr = retval->children_.equal_range(*pc);
            if (cr.first == cr.second) {
                REQUIRE(quiet, "Path navigation failed at step " + std::string(1, step) + ":  child '" + *pc +
                               "' not found in " + ShowPath(here));
                return *FALSE;
            }
            REQUIRE(cr.first == --cr.second,
                    "Path navigation failed at step " + std::string(1, step) + ":  child '" + *pc + "' not unique in " +
                    ShowPath(here));   // Next(cr.first) doesn't work due to compiler bug
