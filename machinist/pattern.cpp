...
namespace {
    ...
    Info_ *NewPattern(const Info_ *parent,
        ...
        std::unique_ptr<Info_> retval(new Info_(parent, parent, varName));

        retval->children_.insert(std::make_pair(PATTERN, Info::MakeLeaf(retval.get(), retval->root_, pattern)));
        retval->children_.insert(std::make_pair(TYPE, Info::MakeLeaf(retval.get(), retval->root_, type)));
        retval->children_.insert(std::make_pair(SECONDVAR, Info::MakeLeaf(retval.get(), retval->root_, secondVar)));
    ...
    ReadPattern(const Info_ *parent,
        ...
        REQUIRE(arrow != std::string::npos, "Input needs type declaration using '->'");
        ...
        auto endVarName = rest.find_first_of(
                " \t");                                    // anything that terminates the variable name
        ...
        auto endSecondVar = rest.find_first_of(
                " \t");                                  // anything that terminates the type
        ...
        if (type == RELDATETYPE) {
            ...
            pattern = std::string(
                    "\\\\d?\\\\d[DMYdmy]|\\\\d{8}");                      // 4 \ required for C++ std string
        }
        if (type == DATETYPE) {
    ...
    struct ParsePattern_ : Info::Parser_ {
        ...
        const {
            ...
            if (*line ==
                PARTS)                                                         // these are the parts that will generate the regular expression
            {
                ++line;
                while (line != content.end()) {
                    ...
                    retval->children_.insert(std::make_pair(ENTRY, thisArg));
                }
            }
            return retval.release();
        }
    ...
    std::string
    ConverterFromString(const Info_ &src) {
    ...
    std::string
    SecondVar(const Info_ &src) {
        return GetMandatory(src, SECONDVAR);
    }


    std::string
    IsRelDate(const Info_ &src) {
        return (GetMandatory(src, TYPE) == RELDATETYPE) ? std::string("1") : std::string();
    }
