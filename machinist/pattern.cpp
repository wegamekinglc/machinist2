...
namespace {
    ...
    vector<string>::const_iterator ReadPattern(const Info_* parent,
    ...
    string SecondVar(const Info_& src) { return GetMandatory(src, SECONDVAR); }

    string IsRelDate(const Info_& src) { return (GetMandatory(src, TYPE) == RELDATETYPE) ? string("1") : string(); }
