...
public:
    ...
    Handle_()
            : std::shared_ptr<const T_>() {}

    Handle_(const T_ *p)
            : std::shared_ptr<const T_>(p) {}

    Handle_(const std::shared_ptr<const T_> &src)
            : std::shared_ptr<const T_>(src) {}
};
