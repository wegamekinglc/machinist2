...
public:
    ...
    virtual vector<string> operator()(const Info_& arg, const Emitter::Funcs_& lib) const = 0;
};
...
public:
    ...
    virtual vector<string> operator()(const string& src, const Emitter::Funcs_& lib) const = 0;
};
...
namespace Emitter {
    ...
    struct Source_ {
        virtual ~Source_();
        virtual Funcs_ Parse(const vector<string>& lib, // contents of library, not paths
                             const string& path) const = 0;
    };
    void RegisterSource(const string& info_type, const Source_& src);

    const Funcs_&
    GetAll(const string& info_type,
