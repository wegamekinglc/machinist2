...
std::vector<std::string>
Emitter::
CallTransform(const std::string &src,
              const Funcs_ &lib,
...
namespace {
    ...
    //
    std::map<std::string, std::vector<const Emitter::Source_ *> > &
    TheSources() {
    ...
    void
    MergeIn(std::map<K_, V_> *dst, const std::map<K_, V_> &contrib) {
       ...
       const std::vector<std::string> &lib)                             // contents of library, not file paths
{
    ...
    if (!RETVALS.count(info_type)) {
        ...
        RETVALS.insert(std::make_pair(info_type, retval));
    }
    std::cout << "Done\n";
