...
if "%ADDRESS_MODEL%" =="Win64" (
cmake -G "%MSVC_VERSION% %ADDRESS_MODEL%" -DCMAKE_BUILD_TYPE=%BUILD_TYPE% -DCMAKE_INSTALL_PREFIX=%MACHINIST_DIR% -DMSVC_RUNTIME=%MSVC_RUNTIME% --target install ..
echo go into this branch
) else (
cmake -G "%MSVC_VERSION%" -DCMAKE_BUILD_TYPE=%BUILD_TYPE% -DCMAKE_INSTALL_PREFIX=%MACHINIST_DIR% -DMSVC_RUNTIME=%MSVC_RUNTIME% --target install ..
)
